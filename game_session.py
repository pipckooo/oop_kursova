...
class GameSession:
    ...
    def get_scores(self):
        ...
        if self.player2:
            ...
            # Логіка індексу: якщо P1 бот, то P2 це індекс 1, інакше 0
    ...
    def _draw_view(self, surface, focus_target, bots_data):
        # 1. Фон (Темно-синій, щоб відрізнити від чорного глюку)
        ...
        # 6. Гравець (якщо це об'єкт Player)
        if hasattr(focus_target, 'draw') and callable(focus_target.draw):
            # Перевіряємо, чи це не наша лямбда-функція (dummy)
            # Лямбда теж callable, але в dummy ми писали lambda *a: None.
            # Player.draw має реальну логіку.
    ...
    def load_save(self, save_data):
        ...
        # Відновлення гравців/ботів не ідеальне для ботів C++,
        # бо ми спавнимо їх на старті. Але для курсової піде перезапуск.
        if self.player1:
